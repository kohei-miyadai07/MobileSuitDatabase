...
class EquipmentRepositoryImplTest {
	...
	class DeleteEquipmentByMsid {
		...
		@Test
		void 指定したモビルスーツIDに紐づいた装備データが削除されていること() {
			sut.deleteEquipmentByMsid("ms3");

			EquipmentRowMapper rowMapper = new EquipmentRowMapper();
			Equipment extend = new Equipment();

			try {
				extend = jdbcTemplate.queryForObject(SELECT_EQUIPMENT_BY_MSID, rowMapper, "ms3");
			} catch(EmptyResultDataAccessException e) {
				extend = null;
			}
	...
	class DeleteEquipmentByArmsId {
		...
		@Test
		void 指定した武器IDに紐づいた装備データが削除されていること() {
			sut.deleteEquipmentByArmsId("arms3");

			EquipmentRowMapper rowMapper = new EquipmentRowMapper();
			Equipment extend = new Equipment();

			try {
				extend = jdbcTemplate.queryForObject(SELECT_EQUIPMENT_BY_ARMSID, rowMapper, "arms3");
			} catch(EmptyResultDataAccessException e) {
				extend = null;
			}
